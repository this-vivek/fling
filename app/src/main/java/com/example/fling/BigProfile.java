...
public class BigProfile extends AppCompatActivity {

    CircleImageView dp;
    TextView following, follower, name, username, hobbies, bio;
    String dFollowing[],dFollower[],dHobbies[];
    String fname,femail,fphone,fbio,fetchedFollower,fetchedFollowing,fhobbies,fimage;
    String imageDP;
    String data;
    Bitmap bitmap;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        progressDialog=new ProgressDialog(BigProfile.this);
        sharedpref();
        ...
        EditProfile.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                i.putExtra("image",fimage);
                i.putExtra("name", fname);
                i.putExtra("username", usernameOLD);
                i.putExtra("followings", fetchedFollowing);
                i.putExtra("followers", fetchedFollower);
                i.putExtra("hobbies", fhobbies);
                i.putExtra("phone", fphone);
                i.putExtra("phone", fphone);
                startActivity(i);
            }
        });
        RecyclerView imagegallery=(RecyclerView)findViewById(R.id.userGallery);
    ...
    private void requestStoragePermission() {
        if (ActivityCompat.shouldShowRequestPermissionRationale(BigProfile.this, android.Manifest.permission.CAMERA)) {
        }
        ActivityCompat.requestPermissions(BigProfile.this, new String[]{Manifest.permission.CAMERA}, CAMERA_REQUEST);
    }
...
/* @Override
    ...
    Data obj=new Data();
        data= URLEncoder.encode("usernameOLD","UTF-8")+"="+URLEncoder.encode(usernameOLD,"UTF-8");
        String data2= obj.selectData(getApplicationContext(),"fetchProfile.php",data);
        JSONObject jsonObject = new JSONObject(data2);
    //init the array;
    //uid1=new String[jsonArray.length()];
        ...
        int i=0;
        while(stringTokenizer.hasMoreTokens()) {
            recevie[i]=stringTokenizer.nextToken("|");
            i++;
        }
        ...
        fimage=recevie[3];
        fhobbies=recevie[4];
        fetchedFollowing=recevie[5];
        fetchedFollower=recevie[6];
        fbio=recevie[7];

    name.setText(fname);
    hobbies.setText(fhobbies);
    bio.setText(fbio);
    follower.setText(fetchedFollower);
    following.setText(fetchedFollowing);
    Log.d("name",fname.toString());
        if(fimage.equals("blank")) {

        }

        else  if(fimage.equals("male"))
        {
            Drawable drawable=getResources().getDrawable(ic_businessman);
            dp.setImageDrawable(drawable);
        }
        else  if(fimage.equals("female"))
        {
    ...
    {
        bitmap = BitmapFactory.decodeStream((InputStream) new URL(fimage).getContent());
